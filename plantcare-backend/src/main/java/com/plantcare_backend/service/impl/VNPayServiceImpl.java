...
public class VNPayServiceImpl implements VNPayService {
    ...
    public String createPaymentUrl(VipOrder order, String ipAddress) {
        ...
        vnp_Params.put("vnp_Amount",
                String.valueOf(order.getAmount().multiply(new java.math.BigDecimal("100")).longValue()));
        ...
        String vnp_SecureHash = generateChecksum(vnp_Params);
    ...
    public boolean verifyPaymentResponse(Map<String, String> response) {
        ...
        // Remove vnp_SecureHash from response for verification
        Map<String, String> paramsForVerification = new HashMap<>(response);
        paramsForVerification.remove("vnp_SecureHash");

        String calculatedHash = generateChecksum(paramsForVerification);
