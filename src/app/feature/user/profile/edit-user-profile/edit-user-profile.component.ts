    ...
    this.http.put<any>(`/api/user/update-avatar`, formData, { withCredentials: true }).subscribe({
      next: (res) => {
        ...
        if (res?.data?.avatar) {
          ...
          this.selectedAvatarFile = null; // Ẩn nút lưu ảnh đại diện
          this.avatarPreview = null; // Ẩn luôn preview tạm thời
        }
        this.toastService.success('Cập nhật ảnh đại diện thành công!');
        this.avatarUploading = false;
        this.cdr.markForCheck();
        this.cdr.detectChanges();
        // Navigate to view profile after successful avatar update
        setTimeout(() => {
    ...
    this.userProfileService.updateUserProfile(updateData).pipe(
      tap(response => {
        ...
        if (response && (response.status === 200 || response.data)) {
          // Cập nhật thông tin user từ response data nếu có
          if (response.data) {
            this.user = { 
              ...this.user, 
              ...response.data 
